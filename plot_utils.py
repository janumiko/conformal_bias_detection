...
def plot_heatmap_of_attendance(
    class_prediction_vectors: dict[int, torch.Tensor],
    save_path: str = None,
    fig_size: tuple[int, int] = (10, 10),
    label_dict: dict[int, str] = IMAGENET_CLASSES_DICT,
) -> None:
    """Plot the classes in a heatmap.

    Args:
        class_prediction_vectors (dict[int, Tensor]): The prediction vectors of the classes
        save_path (Optional[str]): The path to save the plot
        fig_size (tuple[int, int]): The size of the figure
    """
    data = class_prediction_vectors
    categories = list( class_prediction_vectors.keys())

    df = pd.DataFrame.from_dict(class_prediction_vectors)

    grouped = df.apply(lambda col: col[col.isin(categories)].value_counts(), axis=0).fillna(0)

    plt.figure(figsize=fig_size)
    sns.heatmap(grouped, annot=True, cmap='YlGnBu') 
    plt.yticks(ticks=range(len(grouped.index)), labels=[f'{index} \n {label_dict[index].split(" ")[0]}' for index in grouped.index])
    plt.xticks(ticks=range(len(grouped.columns)), labels=[f'{key} \n {label_dict[key].split(" ")[0]}' for key in grouped.columns])
    plt.title('Heatmap of attendace')  
    plt.xlabel('label') 
